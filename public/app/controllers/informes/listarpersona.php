...
// Validar el ID del usuario para evitar inyecciones SQL
if (!filter_var($id_usuario_get, FILTER_VALIDATE_INT)) {
    die('ID inválido.');
}

// Especificar cada campo necesario y usar alias que coinciden con los nombres de las variables en PHP
$sql_personas = "SELECT 
                 ...
                 FROM 
                    tb_personas AS a 
                 INNER JOIN 
                    tb_tipopersonas AS b ON a.id_tipoPersona = b.id_tipoPersona 
                 WHERE 
                    a.id_persona = :id_usuario;";

try {
    $query_personas = $pdo->prepare($sql_personas);
    // Vincular el ID del usuario obtenido por GET como un parámetro seguro
    $query_personas->bindParam(':id_usuario', $id_usuario_get, PDO::PARAM_INT);
    $query_personas->execute();

    $personas_datos = $query_personas->fetchAll(PDO::FETCH_ASSOC);
